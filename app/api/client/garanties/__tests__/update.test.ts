...
describe('PUT /api/client/garanties/update', () => {
  beforeEach(() => vi.clearAllMocks())

  it('devrait mettre à jour une garantie', async () => {
    ...
    const mockGarantie = { id: 'e4bbdf55-5e4c-4ee4-972f-2cc7bd343c55', bassin_id: 'c2ddde77-7e2a-4ef6-994d-4aa9bc261a33' }
    const mockUpdated = { ...mockGarantie, fournisseur: 'Updated' }
    ...
    vi.mocked(supabaseAdmin.from).mockImplementation((table: string) => {
      if (table === 'garanties') {
        return {
          select: vi.fn(() => ({ eq: vi.fn(() => ({ single: vi.fn().mockResolvedValue({ data: { ...mockGarantie, bassins: { batiment_id: 'd3ccef66-6f3b-4ff5-883e-3bb8ad252b44', batiments: { client_id: 'b1ffcd88-8d1a-4df7-aa5c-5aa8ac270b22' } } }, error: null }) })) })),
          update: vi.fn(() => ({ eq: vi.fn(() => ({ select: vi.fn(() => ({ single: vi.fn().mockResolvedValue({ data: mockUpdated, error: null }) })) })) })),
        } as any
      }
      if (table === 'bassins') {
        return { select: vi.fn(() => ({ eq: vi.fn(() => ({ single: vi.fn().mockResolvedValue({ data: { id: 'c2ddde77-7e2a-4ef6-994d-4aa9bc261a33', batiment_id: 'd3ccef66-6f3b-4ff5-883e-3bb8ad252b44', batiments: { client_id: 'b1ffcd88-8d1a-4df7-aa5c-5aa8ac270b22' } }, error: null }) })) })) } as any
      }
      if (table === 'batiments') {
    ...
    const request = new NextRequest('http://localhost:3000/api/client/garanties/update', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer token' },
      body: JSON.stringify({ id: 'e4bbdf55-5e4c-4ee4-972f-2cc7bd343c55', bassinId: 'c2ddde77-7e2a-4ef6-994d-4aa9bc261a33', fournisseur: 'Updated' }),
    })
  ...
  it('devrait rejeter sans authentification', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/client/garanties/update', {
      ...
      body: JSON.stringify({ id: 'e4bbdf55-5e4c-4ee4-972f-2cc7bd343c55', bassinId: 'c2ddde77-7e2a-4ef6-994d-4aa9bc261a33' }),
    })
  ...
  it('devrait valider le schéma Zod', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/client/garanties/update', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer token' },
      body: JSON.stringify({ bassinId: 'c2ddde77-7e2a-4ef6-994d-4aa9bc261a33' }), // id manquant
    })
  ...
  it('devrait rejeter l\'accès à une garantie d\'un autre client', async () => {
    const { requireClient } = await import('@/lib/auth-middleware')
    const { supabaseAdmin } = await import('@/lib/supabaseAdmin')
  ...
  it('devrait retourner 404 si la garantie n\'existe pas', async () => {
    const { requireClient } = await import('@/lib/auth-middleware')
    const { supabaseAdmin } = await import('@/lib/supabaseAdmin')
  ...
  it('devrait retourner 404 si le bassin n\'existe pas', async () => {
    ...
    vi.mocked(supabaseAdmin.from).mockImplementation((table: string) => {
      if (table === 'garanties') {
        return { select: vi.fn(() => ({ eq: vi.fn(() => ({ single: vi.fn().mockResolvedValue({ data: { id: 'e4bbdf55-5e4c-4ee4-972f-2cc7bd343c55', bassin_id: 'c2ddde77-7e2a-4ef6-994d-4aa9bc261a33', bassins: { batiment_id: 'd3ccef66-6f3b-4ff5-883e-3bb8ad252b44', batiments: { client_id: 'b1ffcd88-8d1a-4df7-aa5c-5aa8ac270b22' } } }, error: null }) })) })) } as any
      }
      if (table === 'bassins') {
        return { select: vi.fn(() => ({ eq: vi.fn(() => ({ single: vi.fn().mockResolvedValue({ data: null, error: null }) })) })) } as any
      }
