...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let validated
    try {
      validated = createGarantieSchema.parse(body)
    } catch (error) {
    ...
    const batimentClientId = (bassinExists.batiments as any)?.client_id
    ...
    if (error) {
      return NextResponse.json(
        { error: `Erreur lors de la création de la garantie: ${error.message}` },
        { status: 500 }
      )
    }

    // Notification — notifier admins seulement (le client est l'auteur)
    try {
      ...
      console.error('[NOTIFICATIONS] Erreur notification garantie create (client):', notifError)
    }

    return NextResponse.json({ ok: true, data })
  } catch (e: unknown) {
    logError('API /client/garanties/create', e, { userId: authenticatedUser?.id })
