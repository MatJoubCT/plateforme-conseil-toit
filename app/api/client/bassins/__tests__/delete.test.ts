...
describe('DELETE /api/client/bassins/delete', () => {
  beforeEach(() => vi.clearAllMocks())

  it('devrait supprimer un bassin', async () => {
    ...
    vi.mocked(supabaseAdmin.from).mockImplementation((table: string) => {
      if (table === 'bassins') {
        return {
          select: vi.fn(() => ({ eq: vi.fn(() => ({ single: vi.fn().mockResolvedValue({ data: { id: 'c2ddde77-7e2a-4ef6-994d-4aa9bc261a33', batiment_id: 'd3ccef66-6f3b-4ff5-883e-3bb8ad252b44', batiments: { client_id: 'b1ffcd88-8d1a-4df7-aa5c-5aa8ac270b22' } }, error: null }) })) })),
    ...
    const request = new NextRequest('http://localhost:3000/api/client/bassins/delete', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer token' },
      body: JSON.stringify({ id: 'c2ddde77-7e2a-4ef6-994d-4aa9bc261a33' }),
    })
  ...
  it('devrait rejeter la suppression d\'un bassin introuvable', async () => {
    const { requireClient } = await import('@/lib/auth-middleware')
    const { supabaseAdmin } = await import('@/lib/supabaseAdmin')
