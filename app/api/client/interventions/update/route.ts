...
import { updateInterventionSchema } from '@/lib/schemas/bassin.schema'
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    let validated
    try {
      validated = updateInterventionSchema.parse(body)
    } catch (error) {
    ...
    if (error) {
      return NextResponse.json(
        { error: `Erreur lors de la mise à jour de l'intervention: ${error.message}` },
        { status: 500 }
      )
    }
    ...
    logError('API /client/interventions/update', e, { userId: authenticatedUser?.id })
