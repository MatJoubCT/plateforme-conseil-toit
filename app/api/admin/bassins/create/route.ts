...
import { createBassinSchema } from '@/lib/schemas/bassin.schema'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let validated
    try {
      validated = createBassinSchema.parse(body)
    } catch (error) {
    ...
    logError('API /admin/bassins/create', e, { userId: authenticatedUser?.id })
