...
import { deleteRapportSchema } from '@/lib/schemas/rapport.schema'
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    let validated
    try {
      validated = deleteRapportSchema.parse(body)
    } catch (error) {
    ...
    const { error } = await supabaseAdmin.from('rapports').delete().eq('id', validated.id)

    if (error) {
      return NextResponse.json(
        { error: `Erreur lors de la suppression du rapport: ${error.message}` },
        { status: 500 }
      )
    }
    ...
    logError('API /admin/rapports/delete', e, { userId: authenticatedUser?.id })
