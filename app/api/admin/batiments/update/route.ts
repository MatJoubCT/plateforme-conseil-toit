...
import { updateBatimentSchema } from '@/lib/schemas/batiment.schema'
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    let validated
    try {
      validated = updateBatimentSchema.parse(body)
    } catch (error) {
    ...
    if (error) {
      return NextResponse.json(
        { error: `Erreur lors de la mise à jour du bâtiment: ${error.message}` },
        { status: 500 }
      )
    }
    ...
    logError('API /admin/batiments/update', e, { userId: authenticatedUser?.id })
