...
const deleteBatimentSchema = z.object({
  id: z.string().uuid('ID bâtiment invalide'),
})
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    let validated
    try {
      validated = deleteBatimentSchema.parse(body)
    } catch (error) {
    ...
    // Vérifier s'il y a des bassins associés
    const { count: bassinsCount } = await supabaseAdmin
      .from('bassins')
      .select('id', { count: 'exact', head: true })
      .eq('batiment_id', validated.id)

    if (bassinsCount && bassinsCount > 0) {
      return NextResponse.json(
        {
          error: `Impossible de supprimer ce bâtiment car ${bassinsCount} bassin(s) y sont associés. Supprimez d'abord les bassins.`,
        },
        { status: 409 }
      )
    }
    ...
    const { error } = await supabaseAdmin.from('batiments').delete().eq('id', validated.id)

    if (error) {
      return NextResponse.json(
        { error: `Erreur lors de la suppression du bâtiment: ${error.message}` },
        { status: 500 }
      )
    }
    ...
    logError('API /admin/batiments/delete', e, { userId: authenticatedUser?.id })
