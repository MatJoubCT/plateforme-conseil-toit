...
const deleteMateriauSchema = z.object({
  id: z.string().uuid('ID matériau invalide'),
})
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    let validated
    try {
      validated = deleteMateriauSchema.parse(body)
    } catch (error) {
    ...
    // Suppression du matériau
    const { error } = await supabaseAdmin.from('materiaux').delete().eq('id', validated.id)

    if (error) {
      return NextResponse.json(
        { error: `Erreur lors de la suppression du matériau: ${error.message}` },
        { status: 500 }
      )
    }
    ...
    logError('API /admin/materiaux/delete', e, { userId: authenticatedUser?.id })
