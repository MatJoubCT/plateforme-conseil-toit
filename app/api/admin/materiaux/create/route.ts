...
import { createMateriauSchema } from '@/lib/schemas/materiau.schema'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let validated
    try {
      validated = createMateriauSchema.parse(body)
    } catch (error) {
    ...
    if (error) {
      return NextResponse.json(
        { error: `Erreur lors de la création du matériau: ${error.message}` },
        { status: 500 }
      )
    }
    ...
    logError('API /admin/materiaux/create', e, { userId: authenticatedUser?.id })
